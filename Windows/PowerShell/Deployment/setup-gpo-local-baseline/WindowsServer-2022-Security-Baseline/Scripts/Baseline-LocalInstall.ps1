...
$parentDir = Split-Path -Path $currentScriptPath -Parent

#$GpoMap = .\Tools\MapGuidsToGpoNames.ps1 ..\GPOs
$pathGPOs = "$($parentDir)\GPOs"

$targetScriptTools = Join-Path -Path $currentScriptPath -ChildPath 'Tools\MapGuidsToGpoNames.ps1'

$GpoMap = & $targetScriptTools -rootdir $pathGPOs
...
$GPO_WS_SecureProtocols = "Hardening-Policy: Azure AuditSecureProtocol"
...
if ($WSMember -or $WSNonDomainJoined)
{
    ...
    AddToCollection $GPOs $GPO_WS_SecureProtocols # new from DD
}
